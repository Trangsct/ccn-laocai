...
2. **Chọn ổ**: bot dùng thư mục `du-an` **đã có sẵn** trên ổ D, E, F hoặc G (tạo thư mục đó trước khi cài, ví dụ `F:\du-an` trên máy bàn cơ quan vì ổ D vừa cấm ghi vừa bé); không có thì dùng `C:\du-an`. Mọi file .bat, runner, workflow đều tìm theo cách này. Trong hướng dẫn, chỗ nào ghi `D:\du-an` thì đọc là thư mục `du-an` trên ổ máy đang dùng.
...
1. Mở **PowerShell** (Start → gõ PowerShell → Enter), dán lệnh sau rồi Enter để tải file cài:
   ...
   ```
   $r = 'C:\du-an'; foreach ($o in 'D','E','F','G') { if (Test-Path "$($o):\du-an") { $r = "$($o):\du-an" } }; mkdir "$r\bot" -Force | Out-Null; curl.exe -sSL -o "$r\bot\cai-dat.bat" https://raw.githubusercontent.com/Trangsct/ccn-laocai/main/bot/cai-dat.bat; explorer "$r\bot"
   ```
...
1. Trên máy mới, tải file cài đặt: mở https://raw.githubusercontent.com/Trangsct/ccn-laocai/main/bot/cai-dat.bat
   → bấm chuột phải → *Lưu thành* → lưu vào thư mục `du-an\bot` trên ổ muốn dùng (ví dụ `F:\du-an\bot`; chưa có thì tạo mới; không tạo thì mặc định `C:\du-an\bot`).
